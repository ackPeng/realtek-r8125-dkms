...
//inline void rtl8125_rx_hash(struct rtl8125_private *tp, struct RxDesc *desc, struct sk_buff *skb);
...
static __always_inline void rtl8125_rx_hash_v3(struct rtl8125_private *tp,
        struct RxDescV3 *descv3, struct sk_buff *skb)
{
        u16 rss_header_info = le16_to_cpu(descv3->RxDescNormalDDWord2.HeaderInfo);
        if (likely(rss_header_info & RTL8125_RXS_RSS_L3_TYPE_MASK))
                skb_set_hash(skb, le32_to_cpu(descv3->RxDescNormalDDWord2.RSSResult),
                        (RTL8125_RXS_RSS_L4_TYPE_MASK & rss_header_info) ? PKT_HASH_TYPE_L4 : PKT_HASH_TYPE_L3);
}

static __always_inline void rtl8125_rx_hash_v4(struct rtl8125_private *tp,
        struct RxDescV4 *descv4, struct sk_buff *skb)
{
        u32 rss_header_info = le32_to_cpu(descv4->RxDescNormalDDWord1.RSSInfo);

        if (likely(rss_header_info & RTL8125_RXS_RSS_L3_TYPE_MASK_V4))
                skb_set_hash(skb, le32_to_cpu(descv4->RxDescNormalDDWord1.RSSResult),
                        (RTL8125_RXS_RSS_L4_TYPE_MASK_V4 & rss_header_info) ? PKT_HASH_TYPE_L4 : PKT_HASH_TYPE_L3);
}

static __always_inline void rtl8125_rx_hash(struct rtl8125_private *tp,
        struct RxDesc *desc, struct sk_buff *skb)
{

#if defined(ENABLE_RSS_SUPPORT) || defined(ENABLE_PTP_SUPPORT)
        if (likely(tp->RxDescType == RX_DESC_RING_TYPE_3))
