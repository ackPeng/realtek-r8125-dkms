                                     ...
                                     struct ethtool_rxnfc *cmd)
{
        ...
        case TCP_V4_FLOW:
                ...
                fallthrough;
        case UDP_V4_FLOW:
                if (rtl8125_flag_is_set(tp, RTL_8125_RSS_FLAG_HASH_UDP_IPV4))
                        cmd->data |= RXH_L4_B_0_1 | RXH_L4_B_2_3;
                fallthrough;
        case IPV4_FLOW:
        ...
        case TCP_V6_FLOW:
                ...
                fallthrough;
        case UDP_V6_FLOW:
                if (rtl8125_flag_is_set(tp, RTL_8125_RSS_FLAG_HASH_UDP_IPV6))
                        cmd->data |= RXH_L4_B_0_1 | RXH_L4_B_2_3;
                fallthrough;
        case IPV6_FLOW:
                cmd->data |= RXH_IP_SRC | RXH_IP_DST;
...
#define RTL8125_UDP_RSS_FLAGS (RTL_8125_RSS_FLAG_HASH_UDP_IPV4 | RTL_8125_RSS_FLAG_HASH_UDP_IPV6)
static int _rtl8125_set_rss_hash_opt(struct rtl8125_private *tp)
{
        ...
        hash_mask_len = ilog2(RTL8125_RSS_INDIR_TBL_ENTRIES);
                                    ...
                                    struct ethtool_rxnfc *nfc)
{
        ...
        case UDP_V4_FLOW:
                ...
                case 0:
                        rtl8125_clear_flag(tp, RTL_8125_RSS_FLAG_HASH_UDP_IPV4);
                        break;
                case (RXH_L4_B_0_1 | RXH_L4_B_2_3):
                        rtl8125_set_flag(tp, RTL_8125_RSS_FLAG_HASH_UDP_IPV4);
                        break;
                default:
                        return -EINVAL;
                }
        ...
        case UDP_V6_FLOW:
                ...
                switch (nfc->data & (RXH_L4_B_0_1 | RXH_L4_B_2_3)) {
                case 0:
                        rtl8125_clear_flag(tp, RTL_8125_RSS_FLAG_HASH_UDP_IPV6);
                        break;
                case (RXH_L4_B_0_1 | RXH_L4_B_2_3):
                        rtl8125_set_flag(tp, RTL_8125_RSS_FLAG_HASH_UDP_IPV6);
                        break;
                default:
                        return -EINVAL;
                }
        ...
        rss_ctrl = RTL_R32(tp, RSS_CTRL_8125);

        if (rtl8125_flag_is_set(tp, RTL8125_UDP_RSS_FLAGS))
        //if ((rss_flags & RTL8125_UDP_RSS_FLAGS) && !(tp->rss_flags & RTL8125_UDP_RSS_FLAGS))
                netdev_warn(tp->dev, "enabling UDP RSS: fragmented packets may "
        ...
        rss_ctrl |= RSS_CTRL_TCP_IPV4_SUPP | RSS_CTRL_IPV4_SUPP
                        | RSS_CTRL_IPV6_SUPP | RSS_CTRL_IPV6_EXT_SUPP
...
int rtl8125_set_rxnfc(struct net_device *dev, struct ethtool_rxnfc *cmd)
{
        ...
        //netif_info(tp, drv, dev, "rss set rxnfc\n");
        if (unlikely(!(dev->features & NETIF_F_RXHASH)))
                return 0;
        return (cmd->cmd == ETHTOOL_SRXFH) ?
                rtl8125_set_rss_hash_opt(tp, cmd) : -EOPNOTSUPP;
}
...
u32 rtl8125_get_rxfh_key_size(struct net_device *dev)
{
        ...
        //netif_info(tp, drv, dev, "rss get key size\n");
        if (unlikely(!(dev->features & NETIF_F_RXHASH)))
                return 0;
...
u32 rtl8125_rss_indir_size(struct net_device *dev)
{
        ...
        if (unlikely(!(dev->features & NETIF_F_RXHASH)))
                return 0;
        */
        return RTL8125_RSS_INDIR_TBL_ENTRIES;
}

static inline void rtl8125_get_reta(struct rtl8125_private *tp, u32 *indir)
{
        for (int i = 0; i < RTL8125_RSS_INDIR_TBL_ENTRIES; i++)
...
static void rtl8125_store_reta(struct rtl8125_private *tp)
{
        ...
        for (u8 i = 0; i < RTL8125_RSS_INDIR_TBL_ENTRIES; i++) {
...
{
        u32 rss_key_size = sizeof(tp->rss_key);
        u32 *rss_key = (u32*)tp->rss_key;

        /* Write redirection table to HW */
        for (u32 i = 0; i < rss_key_size; i+=4)
                RTL_W32(tp, RSS_KEY_8125 + i, *rss_key++);
}
...
int rtl8125_get_rxfh(struct net_device *dev, struct ethtool_rxfh_param *rxfh)
{
        ...
        //netif_info(tp, drv, dev, "rss get rxfh\n");
                     ...
                     struct netlink_ext_ack *extack)
{
        ...
        if (rxfh->indir) {
                ...
                /* Verify user input. */
                for (i = 0; i < RTL8125_RSS_INDIR_TBL_ENTRIES; i++)
                     ...
                     const u8 *key, const u8 hfunc)
{
        ...
        netif_info(tp, drv, dev, "rss set rxfh\n");
