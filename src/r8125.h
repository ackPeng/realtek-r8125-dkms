...
// Default is to use FW files
#ifndef DISABLE_USE_FIRMWARE_FILE
#define ENABLE_USE_FIRMWARE_FILE
#endif

// Default is to use HW Close Descriptor
#ifndef DISABLE_TX_NO_CLOSE
#define ENABLE_TX_NO_CLOSE
#endif

// Default is to enable NAPI
#ifndef DISABLE_R8125_NAPI
#define CONFIG_R8125_NAPI
#endif

// Default is to enable VLAN
#ifndef DISABLE_R8125_VLAN
#define CONFIG_R8125_VLAN
#endif
...
#define RTL_KEEP_INTERRUPT_COUNT (10)
...
//#define ENABLE_PAGE_REUSE
#if defined(ENABLE_LARGE_PAGES) || defined(ENABLE_PAGE_REUSE)
#define LP_SUFFIX "-LARGE_PAGES"
#else
#define LP_SUFFIX ""
#endif

#define RTL8125_VERSION "9.014.01" NAPI_SUFFIX REALWOW_SUFFIX PTP_SUFFIX RSS_SUFFIX LP_SUFFIX
...
#define MAX_JUMBO_FRAME_SIZE Jumbo_Frame_9k
...
#define RTL8125_RSS_INDIR_TBL_ENTRIES 128
...
#define RX_BUFFER_SIZE (RX_BUF_SIZE + 1) * 128
#define RX_BUFFER_MASK ((RX_BUFFER_SIZE - 1) & 0xFFFFFF00) // force 256 byte alignment

/* Software IRQ coalescing timer gro_flush_timeout
   Can be changed with "echo 20000 > /sys/class/net/<device>/gro_flush_timeout"
*/
#if NUM_TX_DESC == 2048
#define SW_IRQ_COALESCING_TIMEOUT 20000
#elif NUM_TX_DESC == 4096
#define SW_IRQ_COALESCING_TIMEOUT 25000
#else
#define SW_IRQ_COALESCING_TIMEOUT 20000 
#endif
#define R8125_NAPI_WEIGHT   64 //NUM_RX_DESC / 4

#ifdef ENABLE_DOUBLE_VLAN
#define RX_BUF_SIZE (SZ_16K - 1) //0x05F6  /* 0x05F6(1526) = 1514 + 8(double vlan) + 4(crc) bytes */
#define RT_VALN_HLEN 8      /* 8(double vlan) bytes */
#else
#define RX_BUF_SIZE (SZ_16K - 1) //0x05F2  /* 0x05F2(1522) = 1514 + 4(single vlan) + 4(crc) bytes */
...
#define R8125_MAX_RX_QUEUES R8125_MAX_RX_QUEUES_V2
#define R8125_MAX_QUEUES R8125_MAX_RX_QUEUES

#define OCP_STD_PHY_BASE	0xa400

//Channel Wait Count
#define R8125_CHANNEL_WAIT_COUNT (200)
#define R8125_CHANNEL_WAIT_TIME (5)  // 1us
#define R8125_CHANNEL_EXIT_DELAY_TIME (20)  //20us
#define R8125_CHANNEL_ENTRY_DELAY_TIME (20)  //20us
...
#define RTL_R32_ADDR(tp, reg)	(tp->mmio_addr + (reg))
#define _RTL_R32(addr)		((unsigned long) readl(addr))
...
#define netif_napi_add_weight(ndev,priv,function,weight) netif_napi_add(ndev,priv,function,weight)
#endif
...
#define D3_NO_PLL_DOWN			(BIT(7) | BIT(6))
        ...
        EFUSEAR         = 0xDC,
        CPlusCmd        = 0xE0,
        IntrMitigate    = 0xE2, // On 8125: not Intr. Mitigation
        RxTxFifo        = 0xE2, // On 8125: status of RX/TX FIFO queues (empty or not)
        RDSAR_Q0_LOW_8125 = 0xE4,
        RxDescAddrLow   = 0xE4,
        RxDescAddrHigh  = 0xE8,
        MTPS            = 0xEC,
        FuncEvent       = 0xF0,
...
#define	PFM_D3COLD_EN			(1 << 6)
        ...
        RDSAR_Q2_LOW_8125  = 0x4008,
        RDSAR_Q3_LOW_8125  = 0x4010,
...
enum RTL8125_register_content {
        ...
        RxCfg_9356SEL = (1 << 6),
        EnableRxDescV4_0 = (1 << 1), //not in RxConfigBits but register 0xD8
...
#define RxCSFailMask	(RxIPF | RxUDPF | RxTCPF)
...
#define RxCSFailMask_v3	(RxIPF_v3 | RxUDPF_v3 | RxTCPF_v3 | RxSCTPF_v3)
        ...
        PID1_v4        = (1 << 19), /* Protocol ID bit 1/2 */
        PID0_v4        = (1 << 18), /* Protocol ID bit 2/2 */

#define RxProtoUDP_v4  (PID1_v4)
#define RxProtoTCP_v4  (PID0_v4)
#define RxProtoIP_v4   (PID1_v4 | PID0_v4)
#define RxProtoMask_v4 RxProtoIP_v4
...
#define RxCSFailMask_v4	(RxIPF_v4 | RxUDPF_v4 | RxTCPF_v4 | RxSCTPF_v4)
...
enum r8125_flag {
        R8125_FLAG_DOWN = 28,
        R8125_FLAG_TASK_RESET_PENDING = 29,
        R8125_FLAG_TASK_ESD_CHECK_PENDING = 30,
        R8125_FLAG_TASK_LINKCHG_CHECK_PENDING = 31,
        R8125_FLAG_MAX
};

#define MSI_ATTR RTL_MSI
#define MSIX_ATTR RTL_MSIX
#define WAKEUP_MAGIC_PACKET_V3_ATTR WAKEUP_MAGIC_PACKET_V3
...
enum rtl_flags {
        ...
        RTL_MSI                         = (1 << 3),
        RTL_MSIX                        = (1 << 4),
        WAKEUP_MAGIC_PACKET_V3          = (1 << 5),
        HwHasWrRamCodeToMicroP          = (1 << 6),
        EEPROM_TYPE_93C46               = (1 << 7),     // 128 is also length of EEPROM
        EEPROM_TYPE_93C56               = (1 << 8),     // 256 is also length of EEPROM
        RandomMac                       = (1 << 9),
	TestMode	                = (1 << 10),
        AutoNegMode                     = (1 << 11),
        HwSuppPhyLinkSpeed_2500         = (1 << 12),
        CheckKeepLinkSpeed              = (1 << 13),
        ResumeNoSpeedChange             = (1 << 14),
        RTL_8125_RSS_FLAG_HASH_UDP_IPV4 = (1 << 15),
        RTL_8125_RSS_FLAG_HASH_UDP_IPV6 = (1 << 16),
        UseIntrTimer                    = (1 << 17),
//        RTL_RX_HWTSTAMP_ENABLED         = (1 << 18),
        /*
        R8125_FLAG_DOWN                 = (1 << 28),
        R8125_FLAG_TASK_RESET_PENDING   = (1 << 29),
        R8125_FLAG_TASK_ESD_CHECK_PENDING = (1 << 30),
        R8125_FLAG_TASK_LINKCHG_CHECK_PENDING = (1 << 31)
        */
        // attributes 29-31 reserved for task flags (r8125_flag)
};
...
/* Rx Desc Type -> length calculated as RX_DESC_RING_TYPE_x & 0x3F */
#define RX_DESC_RING_TYPE_1 0x10
#define RX_DESC_RING_TYPE_3 0x20
#define RX_DESC_RING_TYPE_4 0x50

#define R8125_TX_STOP_THRS (MAX_SKB_FRAGS + 1)
#define R8125_TX_START_THRS (2 * R8125_TX_STOP_THRS)
#define netif_subqueue_maybe_stop(dev, idx, get_desc, stop_thrs, start_thrs) \
        netif_txq_maybe_stop(&dev->_tx[idx], get_desc, stop_thrs, start_thrs)

/*Rx Desc Type
enum rx_desc_ring_type {
        RX_DESC_RING_TYPE_1 = 0,
        //RX_DESC_RING_TYPE_2,
        RX_DESC_RING_TYPE_3,
        RX_DESC_RING_TYPE_4,
};
...
#define hw_clo_ptr_reg_v3(ring_index) (u16)(HW_CLO_PTR0_8125 + (ring_index << 2))
#define hw_clo_ptr_reg_v6(ring_index) (u16)(HW_CLO_PTR0_8125BP + (ring_index << 3))
#define sw_tail_ptr_reg_v3(ring_index) (u16)(SW_TAIL_PTR0_8125 + (ring_index << 2))
#define sw_tail_ptr_reg_v6(ring_index) (u16)(SW_TAIL_PTR0_8125BP + (ring_index << 3))
...
#define TX_DESC_ALLOC_SIZE (NUM_TX_DESC * sizeof(struct TxDesc))
#define RX_DESC_ALLOC_SIZE(tp) (NUM_RX_DESC * (tp->RxDescType & 0x3F))

#define rtl8125_rx_config rx_config
#define RxDescDmaPhyAddr RxBufferDmaPhyAddr
//#define RxPhyAddr RxDescArrayPhyAddr
//#define RxDescPhyAddr RxBufferDmaPhyAddr

#ifdef ENABLE_LARGE_PAGES
/*
#define GetRxBufferDmaPhyAddr(tp,ring,ring_index,entry) \
        (tp->RxBufferPhyAddr + (RX_BUF_SIZE + 1) * (NUM_RX_DESC * ring_index + entry))
*/
#define GetRxBufferDmaPhyAddr(ring, entry) (ring->RxBufferPhyAddr + entry * (RX_BUF_SIZE + 1))
//#define GetRxBufferDmaPhyAddr(tp,ring,ring_index,entry) (ring->RxBufferPhyAddr + entry * (RX_BUF_SIZE + 1))

#define GetRxBufferDmaVirtAddr(tp,ring,ring_index,entry) \
        (tp->RxBufferVirtAddr + (RX_BUF_SIZE + 1) * (NUM_RX_DESC * ring_index + entry))
//#define SetRxBufferDmaPhyAddr(ring,entry,mapping)
#else
#define GetRxBufferDmaPhyAddr(ring, entry) ring->RxBufferDmaPhyAddr[entry]
//define GetRxBufferDmaPhyAddr(tp,ring,ring_index,entry) ring->RxBufferDmaPhyAddr[entry]
//#define SetRxBufferDmaPhyAddr(ring,entry,mapping) ring->RxBufferDmaPhyAddr[entry] = mapping
#endif
...
struct rtl8125_rx_ring {
        ...
        void* priv;
        u32 index;
        u64 RxDescDmaPhyAddr[NUM_RX_DESC]; // Rx desc physical address
        u64 RxDescArrayPhyAddr;
        struct sk_buff *Rx_skbuff[NUM_RX_DESC]; // Rx data buffers
#endif

#ifndef ENABLE_LARGE_PAGES
        phys_addr_t RxBufferDmaPhyAddr[NUM_RX_DESC]; // Physical DMA Addresses of RX Ring Buffers
        //struct page *RxBufferPageAddr[NUM_RX_DESC];
#else
        dma_addr_t RxBufferPhyAddr; // Physical DMA Address of single (large) RX Ring Buffer
        u32 RxBufferOffset;
#endif
/*
#ifdef ENABLE_PAGE_REUSE
        struct page *RxDescPageAddr[NUM_RX_DESC]; // Rx desc DMA Physical address, mirrors desc->addr
#endif
*/
};
...
struct rtl8125_private {
        ...
        struct device *device;      /* Cache &pci_dev->dev*/
#ifdef LEGACY_NEW
        struct napi_struct napi[R8125_MAX_MSIX_VEC];
        //struct napi_struct r8125napi[R8125_MAX_MSIX_VEC];
#else
        struct napi_struct napi[R8125_MAX_MSIX_VEC];
#endif
        u8 mcfg;
        ...
        dma_addr_t DescArrayPhyAddr; // Physical DMA Address of RxDesc and TxDesc arrays
        u32 ocp_base;
        //u32 msg_enable;
        u16 cur_page;
        u16 cp_cmd;
...
#define PTP_ROUNDROBIN
#ifdef ENABLE_PTP_SUPPORT
#ifdef PTP_ROUNDROBIN
        u16 ptp_tx_reg;  // 8125 has 4 TX Timestamp registers [0:3]
#endif
        unsigned long ptp_state;
        //u8 EnablePtp;
        //u8 ptp_master_mode;
        s64 ptp_adjust;
	...
	// Protect access to ptp_tx_skb and ptp_tx_start
...
#if defined(ENABLE_RTL_TOOL) || defined(LEGACY)
        u8 rtk_enable_diag;
#endif

#if defined(LEGACY) && !defined(ENABLE_PTP_SUPPORT)
        u8 EnablePtp;
        u8 ptp_master_mode;
#endif
#ifdef ENABLE_ESD
...
#define rtl8125_num_lib_tx_rings(tp) 0
#define rtl8125_num_lib_rx_rings(tp) 0
...
#define TD1_MSS_SHIFT                   18        /* MSS value position */
...
void rtl8125_init_ring_indexes(struct rtl8125_private * const);
u16 rtl8125_ephy_read(const struct rtl8125_private *const, const int);
void rtl8125_ephy_write(const struct rtl8125_private * const, const u8, const u16);
void rtl8125_wait_txrx_fifo_empty(const struct rtl8125_private * const);
void rtl8125_enable_now_is_oob(const struct rtl8125_private * const);
void rtl8125_disable_now_is_oob(const struct rtl8125_private * const);
void rtl8125_hw_disable_mac_mcu_bps(const struct rtl8125_private * const);
void rtl8125_mdio_direct_write_phy_ocp(const struct rtl8125_private * const, const u16, const u16);
u32 rtl8125_mdio_direct_read_phy_ocp(const struct rtl8125_private *const, const u16);

/*
#define rtl8125_clear_and_set_eth_phy_ocp_bit(tp,reg,clearmask,setmask) \
        rtl8125_mdio_direct_write_phy_ocp(tp, reg, \
                (rtl8125_mdio_direct_read_phy_ocp(tp, reg) & ~clearmask) | setmask);
*/

#define rtl8125_clear_mac_ocp_bit(tp,addr,mask) rtl8125_clear_set_mac_ocp_bit(tp,addr,mask,0)
#define rtl8125_set_mac_ocp_bit(tp,addr,mask) rtl8125_clear_set_mac_ocp_bit(tp,addr,0,mask)

static inline u16 rtl8125_mac_ocp_read(const struct rtl8125_private * const tp, const u16 reg_addr)
{
	RTL_W32(tp, MACOCP, reg_addr << 15);
        return (u16)RTL_R32(tp, MACOCP);
}

inline void rtl8125_mark_to_asic(const struct rtl8125_private * const, struct RxDesc * const);

void rtl8125_mark_as_last_descriptor(const struct rtl8125_private * const, const struct RxDesc * const);
void rtl8125_clear_set_mac_ocp_bit(const struct rtl8125_private * const, const u16, const u16, const u16);

#define rtl8125_set_flag(tp,flag) (tp->flags |= flag)
#define rtl8125_clear_flag(tp,flag) (tp->flags &= ~(flag))
#define rtl8125_flag_is_set(tp,flag) (tp->flags & (flag))
#define rtl8125_flag_to_bool(tp,flag) (tp->flags & (flag) ? 1 : 0)

#define is_8125A(tp) ((tp->mcfg & CFG_METHOD_2) == CFG_METHOD_2)
#define is_8125B(tp) ((tp->mcfg & CFG_METHOD_4) == CFG_METHOD_4)
#define is_8125BP(tp) ((tp->mcfg & CFG_METHOD_8) == CFG_METHOD_8)
#define is_8125D(tp) ((tp->mcfg & CFG_METHOD_10) == CFG_METHOD_10)
#define is_8125AB(tp) (tp->mcfg <= CFG_METHOD_5)
#define is_8125BPD(tp) (tp->mcfg >= CFG_METHOD_8)


#define chipset(tp) (tp->mcfg - 2)
#define HW_SUPP_D0_SPEED_UP(tp) (tp->mcfg != CFG_METHOD_2)
//#define HW_SUPP_NUM_TX_QUEUES(tp) (u8)(rtl8125_flag_to_bool(tp, IsMcfg236) ? 1 : 2)
//#define HW_SUPP_NUM_RX_QUEUES(tp) (u8)(rtl8125_flag_to_bool(tp, IsMcfg236) ? 1 : 4)
#define HW_SUPP_NUM_TX_QUEUES(tp) (u8)(is_8125A(tp) ? 1 : 2)
#define HW_SUPP_NUM_RX_QUEUES(tp) (u8)(is_8125A(tp) ? 1 : 4)
#define HW_SUPP_PHY_LINK_SPEED_2500M(tp)        rtl8125_flag_is_set(tp, HwSuppPhyLinkSpeed_2500)
#define HW_HAS_WRITE_PHY_MCU_RAM_CODE(tp)       rtl8125_flag_is_set(tp, HwHasWrRamCodeToMicroP)
